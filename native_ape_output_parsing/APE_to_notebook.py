...
TERM_TYPES = {
    ...
    'StatisticalRelevance': {
        ...
        'NoRelevance',
        'Prediction',
        'MatplotlibObjectRelevance_plain',
        'SklearnObjectRelevance_plain',
        'PandasObjectRelevance_plain',
        'NumpyObjectRelevance_plain',
        'BasicObjectRelevance_plain',
    }
}
...
def get_input_id(param: Dict[str, str|List[str]|List[int]]) -> str:
    # column references
    try:
        if 'Column' in cast(str, param['DataClass']):
            return "'" + cast(List[str], param['APE_label'])[0] + "'"
        # objects
        # inputs
        if cast(int, param['src'][0]) == 0:
            if param['DataClass'] in ['Str']:
                return "'" + cast(List[str], param['APE_label'])[0] + "'"
            return cast(List[str], param['APE_label'])[0]
    except KeyError:
        # Column is not a APELabel but produced by a tool
        assert 'Column' in cast(str, param['DataClass'])
        assert 'APE_label' not in param.keys()
    # tool outputs
    return get_output_id(
        param,
        cast(int, param['src'][0]),
        cast(int, param['src'][1])+1,
    )
...
def get_kwrgs(
    tool: str,
    inputs: List[Dict[str, str|List[str]|List[int]]],
) -> List[Tuple[str, str]]:
    ...
    for input_ix, param in enumerate(inputs):
        try:
            if match_arg(cast(str, param['DataClass']), str(arg_list[arg_ix][1])):
                ...
                raise TypeError(
                    tool + ': '
                    + f'input_ix: {input_ix} - {inputs[input_ix]["DataClass"]}, '
...
def get_function_call(step: Dict, step_num: int) -> str:
    """Get function call string from step"""
    # input params
    if step['tool'] == 'init_sklearn_estimator':
        kwrgs_str = f'estimator="{step["output"][0]["DataClass"]}"'
    elif step['tool'] == 'init_sklearn_searchcv':
        kwrgs_str = f'search_cv="{step["output"][0]["DataClass"]}", ' \
                    + f'estimator="{step["input"][0]["APE_label"][0]}"'
    elif step['tool'] == 'init_sklearn_voting_estimator':
        kwrgs_str = f'voting_estimator="{step["output"][0]["DataClass"]}", ' \
                    + f'estimator_list="{step["input"][0]["APE_label"][0]}"'
    elif step['input'] != []:
        kwrgs_str = ', '.join(
            f'{k}={v}'
            for k, v
            in get_kwrgs(step['tool'], step['input'])
        )
    else:
        raise TypeError(
    ...
    outputs_str = ', '.join(
        get_output_id(param, step_num, ix)
        for ix, param
        in enumerate(step['output'], start=1)
    )
...
- DataClass: `{input['StatisticalRelevance']}`\
    ...
    for num, step in enumerate(cast(List[Dict], solution['steps']), start=input_step+1):
        step_cells.append((
            'markdown',
            f"""\
### Step {num}: `{step['tool']}`
...
#### inputs:
{param_list_to_md(step['input'])}
#### outputs:
{param_list_to_md(step['output'])}\
""",
        ))
        step_cells.append((
            'code',
    ...
    output_cells = [
        ('markdown', f"""\
## Output
{param_list_to_md(cast(List[Dict], solution['output']))}\
    """),
        ('code', ''.join(
            f"display({get_output_id(param, param['src'][0], param['src'][1]+1)})"
            for param
            in cast(List[Dict], solution['output'])
        )),
    ] if cast(List[Dict], solution['output']) else []
